...
from tablip.formats import registry
...
class Dataset:
    ...
    dealing with dozens or hundreds of :class:`Dataset` objects. ::
        ...
        data = tablip.Dataset(*data, headers=headers)
    ...
    def _package(self, dicts=True, ordered=True):
        ...
        # 如果dataset包含表头，则把表头也转为list类型，拼接数据的list
        if self.headers:
            if dicts:
