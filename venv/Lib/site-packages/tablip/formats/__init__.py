...
from tablip.exceptions import UnsupportedFormat
from tablip.utils import normalize_input
...
class Registry:
    ...
    def register(self, key, format_or_path):
        from tablip.core import Databook, Dataset
    ...
    def register_builtins(self):
        ...
        if find_spec('openpyxl'):
            self.register('xlsx', 'tablip.formats._xlsx.XLSXFormat')
        if find_spec('xlrd') and find_spec('xlwt'):
            self.register('xls', 'tablip.formats._xls.XLSFormat')
        if find_spec('yaml'):
            self.register('yaml', 'tablip.formats._yaml.YAMLFormat')
        self.register('csv', CSVFormat())
        self.register('tsv', TSVFormat())
        if find_spec('odf'):
            self.register('ods', 'tablip.formats._ods.ODSFormat')
        self.register('dbf', 'tablip.formats._dbf.DBFFormat')
        if find_spec('MarkupPy'):
            self.register('html', 'tablip.formats._html.HTMLFormat')
        self.register('jira', 'tablip.formats._jira.JIRAFormat')
        self.register('latex', 'tablip.formats._latex.LATEXFormat')
        if find_spec('pandas'):
            self.register('df', 'tablip.formats._df.DataFrameFormat')
        self.register('rst', 'tablip.formats._rst.ReSTFormat')
        if find_spec('tabulate'):
            self.register('cli', 'tablip.formats._cli.CLIFormat')
        ...
        if key not in self._formats:
            if key in uninstalled_format_messages:
                raise UnsupportedFormat(
                    ...
                    "{package_name} (or `pip install tablip[{extras_name}]`).".format(
